...
> [!WARNING]
> **v4.0.0-alpha.3**: this is a pre-release. The API may change before the stable `4.0.0` release. Pin the exact version in your `package.json` and review the [CHANGELOG](https://github.com/TarekRaafat/skalex/blob/master/CHANGELOG.md) before upgrading.
...
- CDN-ready - **ESM** (recommended): `skalex.browser.js` + `src/connectors/storage/browser.js` → `{ LocalStorageAdapter, EncryptedAdapter }`; **IIFE** (quick demos): `<script src="https://cdn.jsdelivr.net/npm/skalex@4.0.0-alpha.3"></script>` → `window.Skalex`
